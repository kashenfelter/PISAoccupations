...
add_iscoII <- function(isco_cols, data) {
    data %>%
        mutate_(ID = 1:dim(data)[1],
                mother_occuII = interp("as.factor(substr(i, 2, 2))", i = as.name(isco_cols[1])),
                father_occuII = interp("as.factor(substr(i, 2, 2))", i = as.name(isco_cols[2]))) %>%
        melt(measure.vars = c("mother_occuII", "father_occuII"), variable.name = "isco_cat_name", value.name = "isco_cat")
